...
module.exports = async (client, guild) => {
  ...
  client.logger.info(`shadow has joined ${guild.name}`);
  ...
  // Self-assign color
  try {
    const shadowColor = guild.roles.cache.find(r => r.name === '#Seagrass');
    if (shadowColor) await guild.me.roles.add(shadowColor);
  } catch (err) {
    client.logger.error(err.message);
  }
